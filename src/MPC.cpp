...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //fg[0] += ; // the sum of the squared cross track errors, + the difference in delta squared, + the difference in a squared
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // The upper and lower limits of delta in radians
  ...
  //for control, we only use steering and acceleration
  return_values.push_back(solution.x[delta_start]);
  return_values.push_back(solution.x[a_start]);

  // for display, we want x and y
  for (i = 0; i < N - 1; i++) {
    return_values.push_back(solution.x[x_start + i + 1]);
    return_values.push_back(solution.x[y_start + i + 1]);
  }
